...
from sqli_crew.crew import SqliCrew
...
def train():
    ...
    try:
        SqliCrew().crew().train(n_iterations=int(sys.argv[1]), filename=sys.argv[2], inputs=inputs)
...
def replay():
    ...
    """
    try:
        SqliCrew().crew().replay(task_id=sys.argv[1])
...
def test():
    ...
    try:
        SqliCrew().crew().test(n_iterations=int(sys.argv[1]), openai_model_name=sys.argv[2], inputs=inputs)
